...
def tabloOlustur():
    ...
    cursor.execute("create table tedarikci_hammadde (id varchar(10), hammadde varchar(5),tedarikci_id varchar(10) ,miktar int,uretim_tarihi varchar(10),raf_omru int,satis_fiyati int, primary key(id))")
    ...
    cursor.execute("create table musteri            (musteri_id varchar(10),musteri_adi varchar(20),adres varchar(50))")
    cursor.execute("create table formuller          (formul varchar(10),bilesen1 varchar(10),bilesen2 varchar(10),bilesen3 varchar(10))")
    cursor.execute("create table ulasim             (id varchar(10),ulke varchar(20),sehir varchar(20),uzaklik int)")
    ...
    cursor.execute("insert into stok_urun(id,kimyasal_urun,uretim_tarihi,raf_omru,iscilik_maliyeti,toplam_maliyet,satis_fiyati) values ('1','CO2','01012019',3,1,400,500)")
    cursor.execute("insert into stok_urun(id,kimyasal_urun,uretim_tarihi,raf_omru,iscilik_maliyeti,toplam_maliyet,satis_fiyati) values ('2','H2O','01012019',4,1,600,750)")
    cursor.execute("insert into stok_urun(id,kimyasal_urun,uretim_tarihi,raf_omru,iscilik_maliyeti,toplam_maliyet,satis_fiyati) values ('3','NH3','01012019',2,1,500,625)")
...
def tedarikci_listele(hammadde,miktar):
    ...
    for j in sonuc:
        tedarikci_liste.append(j[0]+" "*20+str(j[1])+" "*25+str(j[2]))
...
#sorgu = "select tedarikci_id from tedarikci_hammadde where hammadde = '"+hammadde+"' and miktar >= "+miktar
#    sorgu2 = "select floor(if(ulke = 'Turkiye',uzaklik*"+str(yimk)+",uzaklik*"+str(ydmk)+")),sehir\
#             from ulasim"
#    
#    cursor.execute(sorgu2)
#    
...
#    
#    
#    sorgu = "select th.satis_fiyati*"+miktar+"\
#            from tedarikci_hammadde as th\
#            where th.hammadde = '"+hammadde+"' and th.miktar >= "+miktar
#   
#    cursor.execute(sorgu)
#    
...
#        
#        
#   sorgu = "create view satis_maliyetleri as select th.satis_fiyati*"+miktar+"\
#            from tedarikci_hammadde as th\
#            where th.hammadde = '"+hammadde+"' and th.miktar >= "+miktar
#    
#            sorgu = "create view ulasim_maliyetleri as select floor(if(ulke = 'Turkiye',uzaklik*"+str(yimk)+",uzaklik*"+str(ydmk)+"))\
#            from ulasim"
...
def hammadde_satisi(index,miktar,hammadde):
    ...
    sorgu3 = "select miktar from tedarikci_hammadde where tedarikci_id = "+firma_id
    cursor.execute(sorgu3)
    tedarikci_miktar = cursor.fetchall()[0][0]
    print(tedarikci_miktar)
    
    sorgu4 = "select id from stok_hammadde order by id desc limit 1"
    cursor.execute(sorgu4)
    son_id = cursor.fetchall()[0][0]
    print("son id : "+son_id)
    
    sorgu5 = "update tedarikci_hammadde set miktar = "+str(int(tedarikci_miktar)-int(miktar))+" where tedarikci_id = "+firma_id+" and hammadde = '"+hammadde+"'"
...
def kimyasal_uret(hammadde_miktarlar,secilen_hammaddeler,raf_omru,iscilik_maliyeti,toplam_maliyet):
